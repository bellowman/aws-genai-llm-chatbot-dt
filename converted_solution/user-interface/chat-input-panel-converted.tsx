...
import { Model, Workspace, Application } from "../../API";
...
export default function ChatInputPanel(props: ChatInputPanelProps) {
  ...
  const [outputModality, setOutputModality] = useState<ChabotOutputModality>(ChabotOutputModality.Text);
  ...
  const isMediaGenerationModel = (outputModality?: ChabotOutputModality) => {
    ...
    return [ChabotOutputModality.Image, ChabotOutputModality.Video].includes(outputModality);
  };

  // Track active subscription for this session
  const sessionSubscribed = useRef<boolean>(false);
  ...
  useEffect(() => {
    ...
    ws.current.onopen = () => {
      ...
      if (!sessionSubscribed.current) {
        ws.current?.send(JSON.stringify({ action: "subscribe", sessionId: props.session.id }));
        sessionSubscribed.current = true;
      }
      ...
      ws.current?.send(JSON.stringify({ action: "sendmessage", sessionId: props.session.id, message: JSON.stringify(request) }));
    };

    ws.current.onclose = () => setReadyState(ReadyState.CLOSED);
    ws.current.onerror = (err) => {
      setReadyState(ReadyState.CLOSED);
      if (props.setInitErrorMessage) props.setInitErrorMessage("WebSocket error: " + Utils.getErrorMessage(err));
    };
    ...
    ws.current.onmessage = (event) => {
      try {
        ...
        if (msg.sessionId !== props.session.id) return;
    ...
    return () => {
      ws.current?.close();
      sessionSubscribed.current = false;
    };
    ...
    if (props.applicationId) {
      (async () => {
        try {
          ...
          const applicationResult = await apiClient.applications.getApplication(props.applicationId ?? "");
          const application = applicationResult.data?.getApplication;
          if (application) {
            props.setApplication(application);
            setApplication(application);

            const outputModalities = (application.outputModalities ?? []) as ChabotOutputModality[];
      ...
      (async () => {
        ...
        try {
          ...
          if (appContext?.config.rag_enabled) {
            ...
            workspacesStatus = workspacesResult.errors === undefined ? "finished" : "error";
          } else {
          ...
          const selectedModelMetadata = getSelectedModelMetadata(models, selectedModelOption);
          const selectedWorkspace = isMediaGenerationModel(selectedModelMetadata?.outputModalities[0] as ChabotOutputModality)
    ...
    const getSignedUrls = async () => {
      if (props.configuration?.images as SessionFile[]) {
        ...
        for await (const file of props.configuration?.images ?? []) {
          const signedUrl = (await apiClient.sessions.getFileSignedUrl(file.key)).data?.getFileURL;
          if (signedUrl) {
            files.push({
              ...file,
              url: signedUrl,
  ...
  useEffect(() => {
    ...
    const defaultOutputModality = metadata.outputModalities[0] as ChabotOutputModality;
  ...
  const handleSendMessage = async (): Promise<void> => {
    ...
    let name, provider;
    if (!props.applicationId) {
      ({ name, provider } = OptionsHelper.parseValue(state.selectedModel?.value));
    }
                ...
                <Select
                  ...
                  onChange={({ detail }) => {
                    if (detail.selectedOption?.value === "__create__") {
                      ...
                      StorageHelper.setSelectedWorkspaceId(detail.selectedOption?.value ?? "");
                    }
                  }}
...
function getSelectedWorkspaceOption(workspaces: Workspace[]): SelectProps.Option | null {
  ...
  if (savedWorkspaceId) {
    ...
    if (targetWorkspace) {
      selectedWorkspaceOption = OptionsHelper.getSelectOptions([targetWorkspace])[0];
    }
  }
...
function getSelectedModelOption(models: Model[], selectedModel?: string): SelectProps.Option | null {
  ...
  if (!selectedModelOption) {
    ...
    if (!candidate) {
      candidate = bedrockModels.find((m) => m.name === "amazon.titan-tg1-large");
    }
    if (!candidate) {
